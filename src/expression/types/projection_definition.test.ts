...
describe('Projection class', () => {
    ...
    test('should interpolate projections', () => {
        ...
        expect(projection).toStrictEqual(['mercator', 'vertical-perspective', 0.5]);
    });

    test('should serialize to array', () => {
        const projection = ProjectionDefinition.parse(['mercator', 'vertical-perspective', 0.5]);
        expect(`${projection}`).toBe('["mercator","vertical-perspective",0.5]');
        expect(JSON.stringify(projection)).toBe('["mercator","vertical-perspective",0.5]');
        expect(`${projection}` == (JSON.stringify(projection))).toBeTruthy();
    });
});
