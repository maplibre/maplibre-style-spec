...
import {assert, afterEach, describe, test, expect, vi} from 'vitest';
...
// Restore the console.warn spies these tests install so their call counts don't leak between tests.
afterEach(() => {
    vi.restoreAllMocks();
});
...
describe('evaluate expression', () => {
    ...
    test('an auto-wrapped assertion carries its nested index path', () => {
        // bare `["get"]` arms get implicitly wrapped in an assertion that must carry its path
        const {value} = createPropertyExpression(
...
describe('actionable warnings: runtime throw sites carry their index path', () => {
    // wrap the throwing expression at index [1] of `typeof` (no propertySpec, so no fallback suffix) to isolate the prefix
...
describe('actionable warnings: interpolation and legacy functions', () => {
    ...
    test('a nested ["interpolate"] throw carries its index path (mismatched value lengths)', () => {
        // nested so the interpolate's parser key is non-empty (a top-level one would have path '')
    ...
    test('a legacy stop function throw warns with the property location and falls back', () => {
        ...
        expect(console.warn).toHaveBeenCalledTimes(1);
        // the error body is an engine TypeError string; assert only the location prefix
