...
- \`content\`: An array of four numbers, with the first two specifying the left, top corner, and the last two specifying the right, bottom corner. If present, and if the icon uses [\`icon-text-fit\`](${import.meta.env.SERVER_BASE_URL}/layers/#layout-symbol-icon-text-fit), the symbol's text will be fit inside the content box.
...
- \`sdf\`: Boolean. If \`true\` then the image is handled as a signed-distance field (SDF) and its color can be set at runtime using the [\`icon-color\`](${import.meta.env.SERVER_BASE_URL}/layers/#paint-symbol-icon-color) and [\`icon-halo-color\`](${import.meta.env.SERVER_BASE_URL}/layers/#paint-symbol-icon-halo-color) properties. Defaults to \`false\`.
