...
This is the changelog only for the style specification which is shared amongst maplibre implementations. This includes the specification.json and expression tests, but not code of any specific implementation, such as typescript, c++ etc. The specification has not been versioned in a structured way, and keeping a strict CHANGELOG only with the changes of the spec itself is a step towards proper versioning. The below changes are from the npm style spec utilities package which is indepedently versioned - some of these entires are relevant for the spec, some are not because it's implementation specific or related to the utilities.
