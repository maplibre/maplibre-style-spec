...
- Make `url` optional for image sources. An image source can now be defined with only `type` and `coordinates`; it shows nothing until an image is set later, for example with the `updateImage` API in MapLibre GL JS ([#1817](https://github.com/maplibre/maplibre-style-spec/pull/1817))
