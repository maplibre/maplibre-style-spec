...
describe('filter', () => {
    ...
    test('expression, match', () => {
        ...
        expect(match(undefined, { properties: { x: 'a' } })).toBe(true);
        expect(match(undefined, { properties: { x: 'c' } })).toBe(true);
        expect(match(undefined, { properties: { x: 'd' } })).toBe(false);
    });
    ...
    test('expression, within', () => {
        const getPointFromLngLat = (lng, lat, canonical) => {
            ...
            return new Point((p.x * tilesAtZoom - canonical.x) * EXTENT, (p.y * tilesAtZoom - canonical.y) * EXTENT);
        };
        ...
        expect(withinFilter.filter({ zoom: 3 }, { type: 1, geometry: [[getPointFromLngLat(2, 2, canonical)]] }, canonical)).toBe(true);
        expect(withinFilter.filter({ zoom: 3 }, { type: 1, geometry: [[getPointFromLngLat(6, 6, canonical)]] }, canonical)).toBe(false);
        expect(withinFilter.filter({ zoom: 3 }, { type: 1, geometry: [[getPointFromLngLat(5, 5, canonical)]] }, canonical)).toBe(false);
        expect(withinFilter.filter({ zoom: 3 }, { type: 2, geometry: [[getPointFromLngLat(2, 2, canonical), getPointFromLngLat(3, 3, canonical)]] }, canonical)).toBe(true);
        expect(withinFilter.filter({ zoom: 3 }, { type: 2, geometry: [[getPointFromLngLat(6, 6, canonical), getPointFromLngLat(2, 2, canonical)]] }, canonical)).toBe(false);
        expect(withinFilter.filter({ zoom: 3 }, { type: 2, geometry: [[getPointFromLngLat(5, 5, canonical), getPointFromLngLat(2, 2, canonical)]] }, canonical)).toBe(false);
    });
...
//# sourceMappingURL=feature_filter.test.js.map