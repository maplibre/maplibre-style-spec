    ...
    if (Array.isArray(obj.type) || typeof obj.type === 'string') {
        ...
        if (obj.function !== undefined) {
            ...
            if (obj['property-type'] !== 'color-ramp')
                expect(true).toBe(expression.parameters.every(k => k === 'zoom' || k === 'feature' || k === 'feature-state'));
        }
        ...
        obj.forEach((child, j) => {
            if (typeof child === 'string' && scalar.indexOf(child) !== -1)
                return;
        ...
        for (const j in obj)
            validSchema(`${k}.${j}`, v, obj[j], ref, undefined, undefined);
        // Invalid ref object.
    }
    else {
...
//# sourceMappingURL=style-spec.test.js.map